...
class DECMGVIT(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.loss_names = ['G', 'GAN_a', 'GAN_r', 'GAN_s', 'vgg_a', 'vgg_r', 'vgg_s', 'L2_a', 'L2_r', 'L2_s', 'gradient_fake_a', 'gradient_fake_r', 'gradient_fake_s']
        ...
        self.netG = dec_ipt.define_G(opt, self.conv)
    ...
    def forward(self):
        ...
        #print('1 fake R and S', self.fake_R.shape, self.fake_S.shape)
        fake_s = torch.zeros_like(self.fake_R)
        real_s = torch.zeros_like(self.fake_R)
        fake_s[:, 0, :, :], fake_s[:, 1, :, :], fake_s[:, 2, :, :] = self.fake_S, self.fake_S, self.fake_S
        real_s[:, 0, :, :], real_s[:, 1, :, :], real_s[:, 2, :, :] = self.real_S, self.real_S, self.real_S
    ...
    def backward_G(self):
        ...
        # combined loss
        self.loss_GAN = self.loss_GAN_a + self.loss_GAN_r + self.loss_GAN_s
        ...
        self.loss_vgg = self.loss_vgg_a + self.loss_vgg_r + self.loss_vgg_s
        ...
        self.loss_gradient_fake_a = self.criterionMSE(self.gradient_real_A, self.gradient_fake_A)
        self.loss_gradient_fake_r = self.criterionMSE(self.gradient_real_R, self.gradient_fake_R)
        self.loss_gradient_fake_s = self.criterionMSE(self.gradient_real_S, self.gradient_fake_S)
        self.loss_gradient = self.loss_gradient_fake_a + self.loss_gradient_fake_r + self.loss_gradient_fake_s

        self.loss_L2_a = self.criterionIdt(self.real_A, self.fake_A)*2
        #self.loss_L2_at = self.criterionIdt(self.real_A, self.fake_A_com)*2
        self.loss_L2_r = self.criterionIdt(self.real_R, self.fake_R)*2
        self.loss_L2_s = self.criterionIdt(self.real_S, self.fake_S)*2
        self.loss_L2 = self.loss_L2_a + self.loss_L2_r + self.loss_L2_s# + self.loss_L2_at

        #self.loss_ssim_a = (1-self.criterionSSIM(self.real_A, self.fake_A))*2
        #self.loss_ssim_r = (1-self.criterionSSIM(self.real_R, self.fake_R))*2
        #self.loss_ssim_s = (1-self.criterionSSIM(self.real_S, self.fake_S))*2
        #Sself.loss_ssim = self.loss_ssim_a + self.loss_ssim_r + self.loss_ssim_s
