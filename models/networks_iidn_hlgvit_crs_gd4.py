...
class dec_ipt(nn.Module):
    def __init__(self, opt, conv=common.default_conv):
        ...
        sk_conv_d03d = [nn.Conv2d(int(4*n_feats), int(2*n_feats), kernel_size=1, stride=1, padding=0), ActNorm2d(int(2*n_feats)), nn.ReLU(True)]
        self.sk_conv_d03d = nn.Sequential(*sk_conv_d03d)
        sk_conv_d02d = [nn.Conv2d(int(2*n_feats), n_feats, kernel_size=1, stride=1, padding=0), ActNorm2d(n_feats), nn.ReLU(True)]
    ...
    def forward(self, input):
        ...
        d_d_02_in = self.sk_conv_d03d(torch.cat((d_d_03_us, x_e_02),1))
        ...
        d_d_01_in = self.sk_conv_d02d(torch.cat((d_d_02_us, x_e_01),1))
