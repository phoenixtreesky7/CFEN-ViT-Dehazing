...
class DECHLGVIT(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.loss_names = ['G', 'GAN_a', 'GAN_s', 'vgg_a', 'vgg_s', 'L2_a', 'L2_s', 'gradient_fake_a','gradient_fake_s', 'ssim_a', 'ssim_s']
        # specify the images you want to save/display. The program will call base_model.get_current_visuals
        visual_names_A = ['real_A', 'fake_A', 'real_B', 'fake_S', 'real_S']
        ...
        if opt.model_G == 'iid_hlgvit_crs_gd4':
            print('This is the iids_hlgvit_crs_gd4 model')
    ...
    def forward(self):
        # Forward Cycle #
        # disentanglement #
        [self.fake_S, self.fake_A] = self.netG(self.real_B)
        
        fake_s = torch.zeros_like(self.real_B)
        real_s = torch.zeros_like(self.real_B)
    ...
    def backward_G(self):
        ...
        self.loss_GAN = self.loss_GAN_a + self.loss_GAN_s# + self.loss_GAN_s #+ self.loss_GAN_pa
        ...
        self.loss_vgg = self.loss_vgg_a + self.loss_vgg_s# + self.loss_vgg_s #+ self.loss_vgg_pa
        ...
        self.loss_gradient = self.loss_gradient_fake_a + self.loss_gradient_fake_s #+ self.loss_gradient_fake_s #+ self.loss_gradient_fake_pa 
        ...
        self.loss_L2 = self.loss_L2_a + self.loss_L2_s# + self.loss_L2_s #+ self.loss_L2_pa
        ...
        self.loss_ssim = self.loss_ssim_a + self.loss_ssim_s #+ self.loss_ssim_s #+ self.loss_ssim_pa
    ...
    def optimize_parameters(self, opt):
        ...
        self.forward()
        # G_A and G_B
        self.set_requires_grad([self.netD_A, self.netD_S], False)
        ...
        # D_A and D_B
        self.set_requires_grad([self.netD_A, self.netD_S], True)
