...
class dec_ipt(nn.Module):
    def __init__(self, opt, conv=common.default_conv):
        ...
        sk_conv_d03d = [nn.ConvTranspose2d(int(4*n_feats), int(2*n_feats), kernel_size=1, stride=1, padding=0), nn.InstanceNorm2d(int(2*n_feats)), nn.ReLU(True)]
        self.sk_conv_d03d = nn.Sequential(*sk_conv_d03d)
        sk_conv_d02d = [nn.ConvTranspose2d(int(2*n_feats), n_feats, kernel_size=1, stride=1, padding=0), nn.InstanceNorm2d(n_feats), nn.ReLU(True)]
    ...
    def forward(self, input):
        ...
        d_d_02_in = self.sk_conv_d03d(torch.cat((d_d_03_us, r_d_03_us),1))
        ...
        d_d_01_in = self.sk_conv_d02d(torch.cat((d_d_02_us, r_d_02_us),1))
        ...
        #xdh_temp[:,1,:,:] = xr[:,1,:,:]*xs
        #xdh_temp[:,2,:,:] = xr[:,2,:,:]*xs
        xdh = self.sp(torch.cat((input, xr, xd),1))
...
class SpatialPyramid(nn.Module):
    def __init__(self):
        ...
        self.refine1= nn.Conv2d(9, 32, kernel_size=3,stride=1,padding=1)
