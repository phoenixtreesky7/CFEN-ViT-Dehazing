...
def define_G(opt, conv):
    ...
    netG = add_ipt(opt, conv)
    
    return init_net(netG, opt.init_type, opt.gpu_ids)


class add_ipt(nn.Module):
    def __init__(self, opt, def_Conv=common.default_conv):
        super(add_ipt, self).__init__()
        ...
        self.head = nn.ModuleList([
            nn.Sequential(
                def_Conv(opt.n_colors, n_feats, kernel_size),
                common.ResBlock(def_Conv, n_feats, 3, act=act))
        ])
        ...
        self.globalvit_encoder_02 = GViT(img_dim=int((opt.loadSize/2)/opt.l2g_ratio), l2g_ratio=opt.l2g_ratio, patch_dim=int(opt.patch_dim*2), num_channels=int(2*n_feats), embedding_dim=int(2*n_feats)*int(opt.patch_dim*2)*int(opt.patch_dim*2), num_heads=int(opt.num_heads*2), num_layers=opt.num_layers, hidden_dim=int(2*n_feats)*opt.patch_dim*opt.patch_dim*opt.hidden_dim_ratio, num_queries = opt.num_queries, dropout_rate=opt.dropout_rate, mlp=opt.no_mlp ,pos_every=opt.pos_every,no_pos=opt.no_pos,no_norm=opt.no_norm)
        ...
        self.globalvit_encoder_03 = GViT(img_dim=int((opt.loadSize/4)/opt.l2g_ratio), l2g_ratio=opt.l2g_ratio, patch_dim=int(opt.patch_dim*2), num_channels=int(4*n_feats), embedding_dim=int(4*n_feats)*int(opt.patch_dim*2)*int(opt.patch_dim*2), num_heads=int(opt.num_heads*4), num_layers=opt.num_layers, hidden_dim=int(4*n_feats)*int(opt.patch_dim*2)*int(opt.patch_dim*2)*opt.hidden_dim_ratio, num_queries = opt.num_queries, dropout_rate=opt.dropout_rate, mlp=opt.no_mlp ,pos_every=opt.pos_every,no_pos=opt.no_pos,no_norm=opt.no_norm)
        ...
        self.globalvit_decoder_02 = GViT(img_dim=int((opt.loadSize/2)/opt.l2g_ratio), l2g_ratio=opt.l2g_ratio, patch_dim=int(opt.patch_dim*2), num_channels=int(2*n_feats), embedding_dim=int(2*n_feats)*int(opt.patch_dim*2)*int(opt.patch_dim*2), num_heads=int(opt.num_heads*2), num_layers=opt.num_layers, hidden_dim=int(2*n_feats)*int(opt.patch_dim*2)*int(opt.patch_dim*2)*opt.hidden_dim_ratio, num_queries = opt.num_queries, dropout_rate=opt.dropout_rate, mlp=opt.no_mlp ,pos_every=opt.pos_every,no_pos=opt.no_pos,no_norm=opt.no_norm)
        ...
        self.globalvit_decoder_01 = GViT(img_dim=int(opt.loadSize/opt.l2g_ratio), l2g_ratio=opt.l2g_ratio, patch_dim=int(opt.patch_dim*2), num_channels=int(n_feats), embedding_dim=n_feats*int(opt.patch_dim*2)*int(opt.patch_dim*2), num_heads=opt.num_heads, num_layers=opt.num_layers, hidden_dim=n_feats*int(opt.patch_dim*2)*int(opt.patch_dim*2)*opt.hidden_dim_ratio, num_queries = opt.num_queries, dropout_rate=opt.dropout_rate, mlp=opt.no_mlp ,pos_every=opt.pos_every,no_pos=opt.no_pos,no_norm=opt.no_norm)

        self.tail = nn.ModuleList([
            nn.Sequential(
                common.Upsampler(def_Conv, n_feats, act=False),
    ...
    def forward(self, x):
        ...
        [x_lve_01, lu, ld, ru, rd] = self.crop2x2(x)
        ...
        ## Global ViT
        x_gve_01 = self.globalvit_encoder_01(x)

        # Merge the Local ViT and Global ViT, and downscaling
        x_e_01 = x_lve_01+x_gve_01+x
        ...
        x = self.tail[self.scale_idx](x_d_01)
...
class GViT(nn.Module):
    ...
    def forward(self, x, con=False):
        ...
        if self.l2g_ratio[0] == 2:
            x = self.ave_pool2(x)
        elif self.l2g_ratio[0] == 4:
