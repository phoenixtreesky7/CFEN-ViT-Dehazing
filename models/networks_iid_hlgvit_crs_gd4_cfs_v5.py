...
class dec_ipt(nn.Module):
    def __init__(self, opt, conv=common.default_conv):
        ...
        self.head = nn.ModuleList([
            nn.Sequential(
                conv(opt.n_colors, int(n_feats/2), kernel_size),
                common.ResBlock(conv, int(n_feats/2), 3, act=act))
        ])
        ...
        ####################################################################
        ###         Convolution: Local and Global Concatnation           ###
        ####################################################################
        
        ## Encoder ##
        #con_par=[3, 1, 1]
        lgcat_conv_e01 = [nn.Conv2d(int(2*n_feats), int(n_feats), kernel_size=1, stride=1, padding=0), ActNorm2d(int(n_feats)), nn.ReLU(True)]
        self.lgcat_conv_e01 = nn.Sequential(*lgcat_conv_e01)
        lgcat_conv_e02 = [nn.Conv2d(int(4*n_feats), int(2*n_feats), kernel_size=1, stride=1, padding=0), ActNorm2d(int(2*n_feats)), nn.ReLU(True)]
        self.lgcat_conv_e02 = nn.Sequential(*lgcat_conv_e02)
        lgcat_conv_e03 = [nn.Conv2d(int(8*n_feats), int(4*n_feats), kernel_size=1, stride=1, padding=0), ActNorm2d(int(4*n_feats)), nn.ReLU(True)]
        self.lgcat_conv_e03 = nn.Sequential(*lgcat_conv_e03)

        ## Decoder for reflectance image ##
        lgcat_conv_d03r = [nn.Conv2d(int(8*n_feats), int(4*n_feats), kernel_size=1, stride=1, padding=0), ActNorm2d(int(4*n_feats)), nn.ReLU(True)]
        self.lgcat_conv_d03r = nn.Sequential(*lgcat_conv_d03r)
        lgcat_conv_d02r = [nn.Conv2d(int(4*n_feats), int(2*n_feats), kernel_size=1, stride=1, padding=0), ActNorm2d(int(2*n_feats)), nn.ReLU(True)]
        self.lgcat_conv_d02r = nn.Sequential(*lgcat_conv_d02r)
        lgcat_conv_d01r = [nn.Conv2d(int(2*n_feats), int(n_feats), kernel_size=1, stride=1, padding=0), ActNorm2d(int(n_feats)), nn.ReLU(True)]
        self.lgcat_conv_d01r = nn.Sequential(*lgcat_conv_d01r)

        ## Decoder for shading image ##
        lgcat_conv_d03s = [nn.Conv2d(int(8*n_feats), int(4*n_feats), kernel_size=1, stride=1, padding=0), ActNorm2d(int(4*n_feats)), nn.ReLU(True)]
        self.lgcat_conv_d03s = nn.Sequential(*lgcat_conv_d03s)
        lgcat_conv_d02s = [nn.Conv2d(int(4*n_feats), int(2*n_feats), kernel_size=1, stride=1, padding=0), ActNorm2d(int(2*n_feats)), nn.ReLU(True)]
        self.lgcat_conv_d02s = nn.Sequential(*lgcat_conv_d02s)
        lgcat_conv_d01s = [nn.Conv2d(int(2*n_feats), int(n_feats), kernel_size=1, stride=1, padding=0), ActNorm2d(int(n_feats)), nn.ReLU(True)]
        self.lgcat_conv_d01s = nn.Sequential(*lgcat_conv_d01s)

        ## Decoder for dehazed image ##
        lgcat_conv_d03d = [nn.Conv2d(int(8*n_feats), int(4*n_feats), kernel_size=1, stride=1, padding=0), ActNorm2d(int(4*n_feats)), nn.ReLU(True)]
        self.lgcat_conv_d03d = nn.Sequential(*lgcat_conv_d03d)
        lgcat_conv_d02d = [nn.Conv2d(int(4*n_feats), int(2*n_feats), kernel_size=1, stride=1, padding=0), ActNorm2d(int(2*n_feats)), nn.ReLU(True)]
        self.lgcat_conv_d02d = nn.Sequential(*lgcat_conv_d02d)
        lgcat_conv_d01d = [nn.Conv2d(int(2*n_feats), int(n_feats), kernel_size=1, stride=1, padding=0), ActNorm2d(int(n_feats)), nn.ReLU(True)]
        self.lgcat_conv_d01d = nn.Sequential(*lgcat_conv_d01d)
        ...
        ds_conv_e01 = [nn.Conv2d(int(n_feats/2), int(n_feats), kernel_size=3, stride=2, padding=1), nn.InstanceNorm2d(int(n_feats)), nn.ReLU(True)]
        self.ds_conv_e01 = nn.Sequential(*ds_conv_e01)
        ...
        us_conv_d01r = [nn.ConvTranspose2d(int(n_feats), int(n_feats/2), kernel_size=4, stride=2, padding=1), ActNorm2d(int(n_feats/2)), nn.ReLU(True)]
        self.us_conv_d01r = nn.Sequential(*us_conv_d01r)
        ...
        us_conv_d01s = [nn.ConvTranspose2d(int(n_feats), int(n_feats/2), kernel_size=4, stride=2, padding=1), ActNorm2d(int(n_feats/2)), nn.ReLU(True)]
        self.us_conv_d01s = nn.Sequential(*us_conv_d01s)
        ...
        us_conv_d01d = [nn.ConvTranspose2d(int(n_feats), int(n_feats/2), kernel_size=4, stride=2, padding=1), ActNorm2d(int(n_feats/2)), nn.ReLU(True)]
        self.us_conv_d01d = nn.Sequential(*us_conv_d01d)
        ...
        cfsm2g_d03d = [CFSM2G(int(2*n_feats), int(2*n_feats/4))]
        self.cfsm2g_d03d = nn.Sequential(*cfsm2g_d03d)
        cfsm2g_d02d = [CFSM2G(int(n_feats), int(n_feats/4))]
        self.cfsm2g_d02d = nn.Sequential(*cfsm2g_d02d)



        self.tail_R = nn.ModuleList([
            nn.Sequential(
                common.Upsampler(conv, int(n_feats/2), act=False),
                nn.Conv2d(int(n_feats/2), int(n_feats/2), kernel_size=3, stride=1, padding=1), 
                ActNorm2d(int(n_feats/2)), 
                nn.ReLU(True),
                nn.ReflectionPad2d(3),
                nn.Conv2d(int(n_feats/2), opt.n_colors, kernel_size=7, padding=0),
                nn.Tanh()
                #conv(n_feats, opt.n_colors, kernel_size)
            )
        ])
        self.tail_D = nn.ModuleList([
            nn.Sequential(
                common.Upsampler(conv, int(n_feats/2), act=False),
                nn.Conv2d(int(n_feats/2), int(n_feats/2), kernel_size=3, stride=1, padding=1), 
                ActNorm2d(int(n_feats/2)), 
        ...
        self.tail_S = nn.ModuleList([
            nn.Sequential(
                ...
                nn.ReLU(True),
                nn.ReflectionPad2d(3),
                nn.Conv2d(int(n_feats/2), 1, kernel_size=7, padding=0),
                nn.Tanh()
                #conv(n_feats, opt.n_colors, kernel_size)
            )
        ])
    ...
    def forward(self, input):
        ...
        x_e_01 = self.lgcat_conv_e01(torch.cat((x_lve_01, x_gve_01),1)) + xf
        ...
        # Merge the Local ViT and Global ViT, and downscaling
        x_e_02 = self.lgcat_conv_e02(torch.cat((x_lve_02, x_gve_02),1)) + x_e_01_ds
        ...
        x_e_03 = self.lgcat_conv_e03(torch.cat((x_lve_03, x_gve_03),1)) + x_e_02_ds
        ...
        r_d_03 = self.lgcat_conv_d03r(torch.cat((r_lvd_03, r_gvd_03),1)) + r_d_03_in
        ...
        # Merge the Local ViT and Global ViT, and upscaling
        r_d_02 = self.lgcat_conv_d02r(torch.cat((r_lvd_02, r_gvd_02),1)) + r_d_02_in
        ...
        # Merge the Local ViT and Global ViT, and downscaling
        r_d_01 = self.lgcat_conv_d01r(torch.cat((r_lvd_01, r_gvd_01),1)) + r_d_01_in
        

        xr = self.us_conv_d01r(r_d_01+xf)
        xr = self.tail_R[self.scale_idx](xr)
        #xr = self.upsam(xr)
        ...
        # Merge the Local ViT and Global ViT, and upscaling
        s_d_03 = self.lgcat_conv_d03s(torch.cat((s_lvd_03, s_gvd_03),1)) + s_d_03_in
        ...
        # Merge the Local ViT and Global ViT, and upscaling
        s_d_02 = self.lgcat_conv_d02s(torch.cat((s_lvd_02, s_gvd_02),1)) + s_d_02_in
        ...
        # Merge the Local ViT and Global ViT, and downscaling
        s_d_01 = self.lgcat_conv_d01s(torch.cat((s_lvd_01, s_gvd_01),1)) + s_d_01_in

        xs = self.us_conv_d01s(s_d_01+xf)
        xs = self.tail_S[self.scale_idx](xs)
        #xs = self.upsam(xs)
        #print('tail(x)', x.shape)
        #x = self.add_mean(x)
        ...
        # Merge the Local ViT and Global ViT, and upscaling
        d_d_03 = self.lgcat_conv_d03d(torch.cat((d_lvd_03, d_gvd_03),1)) + d_d_03_in
        
        ### Decoder Level Two ###
        ## Loval ViT
        # Origional feature maps are cropped to 2x2 regions
        d_d_03_us = self.us_conv_d03d(d_d_03) 
        d_d_02_in = self.cfsm2g_d03d([d_d_03_us, r_d_03_us, s_d_03_us])
        ...
        # Merge the Local ViT and Global ViT, and upscaling
        d_d_02 = self.lgcat_conv_d02d(torch.cat((d_lvd_02, d_gvd_02),1)) + d_d_02_in


        ### Encoder Level One ###
        ## Local ViT ##
        # Origional feature maps are cropped to 2x2 regions
        d_d_02_us = self.us_conv_d02d(d_d_02) 
        d_d_01_in = self.cfsm2g_d02d([d_d_02_us, r_d_02_us, s_d_02_us])
        ...
        # Merge the Local ViT and Global ViT, and downscaling
        d_d_01 = self.lgcat_conv_d01d(torch.cat((d_lvd_01, d_gvd_01),1)) + d_d_01_in

        xd = self.us_conv_d01d(d_d_01+xf)
        xd = self.tail_D[self.scale_idx](xd)
        #xd = self.upsam(xd)
        #print('tail(x)', x.shape)
        #x = self.add_mean(x)
...
class LViT(nn.Module):
    def __init__(
        ...
        self.flatten_dim = patch_dim * patch_dim * num_channels//4
        
        self.out_dim = patch_dim * patch_dim * num_channels//4
        
        self.no_pos = no_pos
        
        conv_shrink = [nn.Conv2d(num_channels, int(num_channels/4), kernel_size=1, stride=1, padding=0), ActNorm2d(int(num_channels/4)), nn.ReLU(True)]
        self.conv_shrink = nn.Sequential(*conv_shrink)
        conv_extend = [nn.Conv2d(int(num_channels/4), num_channels, kernel_size=1, stride=1, padding=0), ActNorm2d(num_channels), nn.ReLU(True)]
        self.conv_extend = nn.Sequential(*conv_extend)

        self.num_channels = int(num_channels/4)
        ...
        if self.mlp==False:
            ...
            self.query_embed = nn.Embedding(num_queries, self.embedding_dim * self.seq_length)

        encoder_layer = TransformerEncoderLayer(self.embedding_dim, num_heads, self.hidden_dim, dropout_rate, self.no_norm)
        self.encoder = TransformerEncoder(encoder_layer, num_layers)
        
        decoder_layer = TransformerDecoderLayer(self.embedding_dim, num_heads, self.hidden_dim, dropout_rate, self.no_norm)
...
# complementary features selection module: 2 group of complementary features
class CFSM2G(nn.Module):
    def __init__(self, in_nc, bk_nc):
        super(CFSM2G, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.max_pool = nn.AdaptiveMaxPool2d(1)
           
        self.fc_avg_cf1 = nn.Sequential(nn.Conv2d(in_nc, bk_nc, 1, bias=False),
                               nn.ReLU(),
                               nn.Conv2d(bk_nc, in_nc, 1, bias=False))
        self.fc_avg_cf2 = nn.Sequential(nn.Conv2d(in_nc, bk_nc, 1, bias=False),
                               nn.ReLU(),
                               nn.Conv2d(bk_nc, in_nc, 1, bias=False))
        self.fc_max_cf1 = nn.Sequential(nn.Conv2d(in_nc, bk_nc, 1, bias=False),
                               nn.ReLU(),
                               nn.Conv2d(bk_nc, in_nc, 1, bias=False))
        self.fc_max_cf2 = nn.Sequential(nn.Conv2d(in_nc, bk_nc, 1, bias=False),
                               nn.ReLU(),
                               nn.Conv2d(bk_nc, in_nc, 1, bias=False))
    ...
    def forward(self, x):
        comb = x[0] + x[1] + x[2]
        globalavg_comb = self.avg_pool(comb)
        globalmax_comb = self.max_pool(comb)

        avg_out_cf1 = self.fc_avg_cf1(globalavg_comb)
        max_out_cf1 = self.fc_max_cf1(globalmax_comb)
        sig_cf1 = self.sigmoid(avg_out_cf1 + max_out_cf1)

        avg_out_cf2 = self.fc_avg_cf2(globalavg_comb)
        max_out_cf2 = self.fc_max_cf2(globalmax_comb)
        sig_cf2 = self.sigmoid(avg_out_cf2 + max_out_cf2)
