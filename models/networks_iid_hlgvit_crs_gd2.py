...
class dec_ipt(nn.Module):
    def __init__(self, opt, conv=common.default_conv):
        ...
        ## Encoder ##
        #con_par=[3, 1, 1]
        lgcat_conv_e01 = [nn.Conv2d(int(2*n_feats), int(n_feats), kernel_size=1, stride=1, padding=0), nn.InstanceNorm2d(int(n_feats)), nn.ReLU(True)]
        self.lgcat_conv_e01 = nn.Sequential(*lgcat_conv_e01)
        lgcat_conv_e02 = [nn.Conv2d(int(4*n_feats), int(2*n_feats), kernel_size=1, stride=1, padding=0), nn.InstanceNorm2d(int(2*n_feats)), nn.ReLU(True)]
        self.lgcat_conv_e02 = nn.Sequential(*lgcat_conv_e02)
        lgcat_conv_e03 = [nn.Conv2d(int(8*n_feats), int(4*n_feats), kernel_size=1, stride=1, padding=0), nn.InstanceNorm2d(int(4*n_feats)), nn.ReLU(True)]
        self.lgcat_conv_e03 = nn.Sequential(*lgcat_conv_e03)

        ## Decoder for reflectance image ##
        lgcat_conv_d03r = [nn.Conv2d(int(8*n_feats), int(4*n_feats), kernel_size=1, stride=1, padding=0), nn.InstanceNorm2d(int(4*n_feats)), nn.ReLU(True)]
        self.lgcat_conv_d03r = nn.Sequential(*lgcat_conv_d03r)
        lgcat_conv_d02r = [nn.Conv2d(int(4*n_feats), int(2*n_feats), kernel_size=1, stride=1, padding=0), nn.InstanceNorm2d(int(2*n_feats)), nn.ReLU(True)]
        self.lgcat_conv_d02r = nn.Sequential(*lgcat_conv_d02r)
        lgcat_conv_d01r = [nn.Conv2d(int(2*n_feats), int(n_feats), kernel_size=1, stride=1, padding=0), nn.InstanceNorm2d(int(n_feats)), nn.ReLU(True)]
        self.lgcat_conv_d01r = nn.Sequential(*lgcat_conv_d01r)

        ## Decoder for shading image ##
        lgcat_conv_d03s = [nn.Conv2d(int(8*n_feats), int(4*n_feats), kernel_size=1, stride=1, padding=0), nn.InstanceNorm2d(int(4*n_feats)), nn.ReLU(True)]
        self.lgcat_conv_d03s = nn.Sequential(*lgcat_conv_d03s)
        lgcat_conv_d02s = [nn.Conv2d(int(4*n_feats), int(2*n_feats), kernel_size=1, stride=1, padding=0), nn.InstanceNorm2d(int(2*n_feats)), nn.ReLU(True)]
        self.lgcat_conv_d02s = nn.Sequential(*lgcat_conv_d02s)
        lgcat_conv_d01s = [nn.Conv2d(int(2*n_feats), int(n_feats), kernel_size=1, stride=1, padding=0), nn.InstanceNorm2d(int(n_feats)), nn.ReLU(True)]
        self.lgcat_conv_d01s = nn.Sequential(*lgcat_conv_d01s)

        ## Decoder for dehazed image ##
        lgcat_conv_d03d = [nn.Conv2d(int(8*n_feats), int(4*n_feats), kernel_size=1, stride=1, padding=0), nn.InstanceNorm2d(int(4*n_feats)), nn.ReLU(True)]
        self.lgcat_conv_d03d = nn.Sequential(*lgcat_conv_d03d)
        lgcat_conv_d02d = [nn.Conv2d(int(4*n_feats), int(2*n_feats), kernel_size=1, stride=1, padding=0), nn.InstanceNorm2d(int(2*n_feats)), nn.ReLU(True)]
        self.lgcat_conv_d02d = nn.Sequential(*lgcat_conv_d02d)
        lgcat_conv_d01d = [nn.Conv2d(int(2*n_feats), int(n_feats), kernel_size=1, stride=1, padding=0), nn.InstanceNorm2d(int(n_feats)), nn.ReLU(True)]
        ...
        us_conv_d02r = [nn.ConvTranspose2d(int(2*n_feats), n_feats, kernel_size=4, stride=2, padding=1), nn.InstanceNorm2d(n_feats), nn.ReLU(True)]
        ...
        us_conv_d02s = [nn.ConvTranspose2d(int(2*n_feats), n_feats, kernel_size=4, stride=2, padding=1), nn.InstanceNorm2d(n_feats), nn.ReLU(True)]
        ...
        us_conv_d02d = [nn.ConvTranspose2d(int(2*n_feats), n_feats, kernel_size=4, stride=2, padding=1), nn.InstanceNorm2d(n_feats), nn.ReLU(True)]
        ...
        sk_conv_d03r = [nn.ConvTranspose2d(int(4*n_feats), int(2*n_feats), kernel_size=1, stride=1, padding=0), nn.InstanceNorm2d(int(2*n_feats)), nn.ReLU(True)]
        self.sk_conv_d03r = nn.Sequential(*sk_conv_d03r)
        sk_conv_d02r = [nn.ConvTranspose2d(int(2*n_feats), n_feats, kernel_size=1, stride=1, padding=0), nn.InstanceNorm2d(n_feats), nn.ReLU(True)]
        self.sk_conv_d02r = nn.Sequential(*sk_conv_d02r)

        ## Skip-conection Concatnation Convolution (shading) ##
        sk_conv_d03s = [nn.ConvTranspose2d(int(4*n_feats), int(2*n_feats), kernel_size=1, stride=1, padding=0), nn.InstanceNorm2d(int(2*n_feats)), nn.ReLU(True)]
        self.sk_conv_d03s = nn.Sequential(*sk_conv_d03s)
        sk_conv_d02s = [nn.ConvTranspose2d(int(2*n_feats), n_feats, kernel_size=1, stride=1, padding=0), nn.InstanceNorm2d(n_feats), nn.ReLU(True)]
        self.sk_conv_d02s = nn.Sequential(*sk_conv_d02s)

        ## Skip-conection Concatnation Convolution (dehazed) ##
        sk_conv_d03d = [nn.ConvTranspose2d(int(6*n_feats), int(2*n_feats), kernel_size=1, stride=1, padding=0), nn.InstanceNorm2d(int(2*n_feats)), nn.ReLU(True)]
        self.sk_conv_d03d = nn.Sequential(*sk_conv_d03d)
        sk_conv_d02d = [nn.ConvTranspose2d(int(3*n_feats), n_feats, kernel_size=1, stride=1, padding=0), nn.InstanceNorm2d(n_feats), nn.ReLU(True)]
...
class basic_Conv2d(nn.Module):
    def __init__(self, input_channle, output_channle, kernel_size=3, stride=1, padding=1, norm="instance",
        ...
        self.norm=nn.nn.InstanceNorm2d(output_channle, affine=True)
